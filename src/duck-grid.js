...
class DuckGrid extends HTMLElement {
  ...
  // ── schema detection ──────────────────────────────────────────────────────
  async #detectSchema() {
    const res = await this.#conn.query(`DESCRIBE "${this.#viewName}"`);
    this.#schema = res.toArray().map((r) => {
      //changed
      const kind = this.#kindFromType(r.column_type);

      return {
        name: r.column_name,
        rawType: r.column_type.toUpperCase(),
        kind,
        operators: this.#operatorsForKind(kind),
      };
    });
  ...
  #buildWhere() {
    ...
    for (const [column, filters] of Object.entries(this.#filters)) {
      const meta = this.#schema.find((c) => c.name === column);

      if (!meta) continue;

      for (const filter of filters) {
        const sql = this.#conditionToSql(column, meta.kind, filter);
  ...
  #conditionToSql(column, kind, condition) {
  ...
  #renderTable(rows, cols) {
    ...
    thead.innerHTML = `<tr>${cols
      .map((c) => {
        ...
        const active = this.#filters[c]?.length > 0;
        return `
        <th data-col="${c}" class="${sorted ? "dg-sorted" : ""}">
        <div class="dg-header">
          <span>${c.replace(/_/g, " ")}</span>
          <button
              class="dg-filter-btn" ${active ? "active" : ""}"
              data-filter-col="${c}">
              <svg
                viewBox="0 0 24 24"
                width="14"
    ...
    tbody.innerHTML = rows
      .map(
        (row) =>
          `<tr>${cols.map((c) => this.#renderCell(row[c])).join("")}</tr>`,
      )
      .join("");

    table.style.display = "";
    this.#id_el("count").textContent =
      `${this.#totalRows.toLocaleString()} rows`;
